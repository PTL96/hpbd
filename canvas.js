...
(function () {
    ...
    function drawBalloon(x, y, balloonImage) {
      ctx.drawImage(balloonImage, x, y, balloonImage.width, balloonImage.height);
    }
    ...
    //   balloonImage.src = 'https://static.jennyparty.vn/post/news/2017/6-9/huong-dan-trang-tri-sinh-nhat-voi-bong-bong-8.jpg'; // Thay đổi đường dẫn đến hình ảnh bóng bay của bạn
    
    //   // Vẽ bóng bay tại vị trí ngẫu nhiên
    //   let numBalloons = 50; // Số lượng hình ảnh bóng bay bạn muốn vẽ
    //   for (let i = 0; i < numBalloons; i++) {
    //     let balloonX = getRandomInt(0, canvas.width - balloonImage.width);
    //     let balloonY = getRandomInt(0, canvas.height - balloonImage.height);
    //     drawBalloon(balloonX, balloonY, balloonImage);
    //   }
    // Tạo mảng chứa các đường dẫn đến hình ảnh bóng bay
let balloonImages = [
  'https://bizweb.dktcdn.net/100/436/204/products/c8627b69-85f2-4a37-932b-b63c871b93d4.jpg?v=1632565784293',
  'https://www.partystore.vn/wp-content/uploads/2021/10/PD61-Set-bong-bong-trang-tri-sinh-nhat-be-trai-chu-de-Phi-Hanh-Gia-Phi-Thuyen-day-chu-Happy-Birthday-2.jpg',
  'https://bongbongdep.com/images/201911/goods_img/set-bong-bong-sinh-nhat-danh-cho-be-trai-sieu-nhan-boy3-P3887-1574650676521.jpg'
];
...
// Trong hàm drawFlowers
function drawFlowers() {
  scaleCanvas();
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  let balloonWidth = 100;
  let balloonHeight = 150; 
  let numBalloons = 50; 
  for (let i = 0; i < numBalloons; i++) {
    let balloonImage = new Image();
    balloonImage.src = getRandomBalloonImage();
    balloonImage.onload = function() {
      let balloonX = getRandomInt(0, canvas.width - balloonImage.width);
      let balloonY = getRandomInt(0, canvas.height - balloonImage.height);
      if (balloonX + balloonWidth > canvas.width) {
        balloonX = canvas.width - balloonWidth;
      }
      if (balloonY + balloonHeight > canvas.height) {
        balloonY = canvas.height - balloonHeight;
      }
      drawBalloon(balloonX, balloonY, balloonImage);
    };
  }
